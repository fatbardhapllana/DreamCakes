...
export default function CakeEdit({ cake }: Props) {
    ...
    const [preview, setPreview] = useState<string | null>(
        cake.image ? `/storage/${cake.image}` : null
    );
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        router.post(`/cakes/${cake.id}`, formData, {
    ...
    return (
        <AppLayout>
            <div className="p-8 max-w-2xl mx-auto">
                <h1 className="text-3xl font-bold mb-8">✏️ Edito Tortën</h1>
                    ...
                    <div>
                        ...
                        {preview && (
                            <img src={preview} alt="Preview"
                                className="mb-3 w-full h-56 object-cover rounded-lg border border-gray-700" />
                        )}
                        <label className="flex flex-col items-center justify-center w-full bg-gray-800 border border-gray-700 border-dashed rounded-lg px-4 py-8 cursor-pointer hover:border-pink-500 transition">
                            <p className="text-gray-400 text-sm">📷 Zgjidh foton e re të tortës</p>
